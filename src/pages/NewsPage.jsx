...
export default function NewsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      {/* Карточки статистики */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-5 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 p-2 md:p-5 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[#718ebf] text-xs md:text-sm">Всего новостей</p>
              <p className="text-2xl font-bold text-[#1a1f36] mt-1">{stats.total}</p>
            </div>
            <div className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-blue-50 flex items-center justify-center">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-2 md:p-5 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[#718ebf] text-xs md:text-sm">Опубликовано</p>
              <p className="text-2xl font-bold text-green-600 mt-1">{stats.published}</p>
            </div>
            <div className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-green-50 flex items-center justify-center">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-2 md:p-5 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[#718ebf] text-xs md:text-sm">Черновики</p>
              <p className="text-2xl font-bold text-[#feaa09] mt-1">{stats.draft}</p>
            </div>
            <div className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-yellow-50 flex items-center justify-center">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-2 md:p-5 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[#718ebf] text-xs md:text-sm">Всего просмотров</p>
              <p className="text-2xl font-bold text-[#fe5c73] mt-1">{stats.views}</p>
            </div>
            <div className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-pink-50 flex items-center justify-center">
      ...
      <div className="flex flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        {canCreate && (
          ...
          >
            ...
            <span className='hidden sm:flex'>Добавить новость</span>
          </button>
        )}
      </div>
      ...
      <div className="grid gap-4">
        {news.length === 0 ? (
          ...
          news.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl border border-gray-100 p-2 md:p-3 lg:p-4 hover:shadow-md transition-all duration-200">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex gap-4 flex-1">
                  <div className="hidden sm:block w-8 h-8 md:w-10 md:h-10 rounded-xl bg-gray-50 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-[#718ebf]">
                      ...
                      <span className="flex items-center gap-1">{item.views} просмотров</span>
                    </div>
                  </div>
                </div>
